...
def main():
    ...
    while current_iter <= total_iters:
        ...
        while train_data is not None:
            ...
            if 'distance' in train_data:
                model.feed_train_data({'lq': lq, 'gt': gt, 'distance': train_data['distance']})
            elif 'psnr_cls' in train_data:
                model.feed_train_data({'lq': lq, 'gt': gt, 'psnr_cls': train_data['psnr_cls']})
            elif 'lr' in train_data:
                model.feed_train_data({'lq': lq, 'gt': gt, 'lr': lr})
            else:
                model.feed_train_data({'lq': lq, 'gt':gt})
            model.optimize_parameters(current_iter) 
