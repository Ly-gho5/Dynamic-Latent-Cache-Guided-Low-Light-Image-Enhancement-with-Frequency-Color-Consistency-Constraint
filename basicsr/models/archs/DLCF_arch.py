...
class DLCF(nn.Module):
    def __init__(self, 
        ...
        # 在 HINT1 中增加一个全局颜色调整模块
        self.color_correct = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),          # 全局池化得到 [B, C, 1, 1]
            nn.Conv2d(refinement_dim, dim // 4, 1),      # 压缩
            nn.ReLU(),
            nn.Conv2d(dim // 4, 3, 1)         # 输出 RGB 偏移量 [B, 3, 1, 1]
        )
    ...
    def forward(self, inp_img):
        ...
        ###########################
            ...
            out_residual = out_dec_level1 
            color_bias = self.color_correct(out_residual) # 计算全局颜色偏差
            out_final = self.output(out_residual) + inp_img + color_bias # 加上这个偏差
